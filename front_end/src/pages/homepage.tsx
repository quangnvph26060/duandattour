...
const HomePage = () => {
  ...
  const getListDiemDen = async () => {
    try {
      const response = await axios.get(
        "http://127.0.0.1:8000/api/getListDiemDen"
      );
  ...
  const getListDiemDi = async () => {
    try {
      const response = await axios.get(
        "http://127.0.0.1:8000/api/getListDiemDi"
      );
      console.log(response.data.data);
  ...
  // boxchat 
  const [maxExpiryDates, setMaxExpiryDates] = useState(0);
  const [remainingTime, setRemainingTime] = useState(maxExpiryDates);
  ...
  function formatTime(time) {
    const days = Math.floor(time / (24 * 60 * 60)); // Số ngày
    const hours = Math.floor((time % (24 * 60 * 60)) / (60 * 60)); // Số giờ
    const minutes = Math.floor((time % (60 * 60)) / 60); // Số phút
    const seconds = time % 60; // Số giây
    ...
    return `${formattedDays} ngày ${formattedHours}:${formattedMinutes}:${formattedSeconds}`;
  }
  
  const [tourKM, setTourKM] = useState([]);

  useEffect(() => {
    fetch("http://127.0.0.1:8000/api/listtourKM")
      .then((response) => response.json())
      .then((result) => {
        setTourKM(result.tourKM);
        console.log(result.tourKM);
  
        const currentDate = new Date();
        const expiryDates = result.tourKM.map((tour) => {
          const expiryDate = new Date(tour.max_expiry_date);
  ...
  return (
    <div className="bg-white rounded-lg shadow p-4">
      ...
      <div
        className="bg-white box-shadow rounded-lg  p-9 mx-auto"
        style={{ width: "1100px" }}
      >
          ...
          <div className="flex items-center mr-4">
            ...
            >
              ...
              {DiemDenData.map((DiemDen) => (
                <option value={DiemDen} key={DiemDen}>
                  {DiemDen}
                </option>
              ))}
          ...
          <div className="flex items-center mr-4">
            ...
            >
              ...
              {DiemDiData.map((DiemDi) => (
                <option value={DiemDi} key={DiemDi}>
                  {DiemDi}
                </option>
              ))}
          ...
          {tourKM.map((item) => (
            ...
            >
              ...
              <div className="product-details mt-4">
                ...
                <p className="price">Giá :1500000đ</p>
                <p
                  style={{
                    color: "#fd5056",
                    fontSize: "18px",
