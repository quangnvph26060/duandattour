...
const AdminProduct = (props: Props) => {
  ...
  const hdvtour = () => {
    axios.get("http://127.0.0.1:8000/api/admin/hdvtour").then((response) => {
      console.log(response.data.hdv.name);
      setHdvData(response.data.hdv);
    });
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (ma_loai_tour: number) => {
        const loaiTour = loaitourArrary.find(
          (item: { id: number }) => item.id === ma_loai_tour
        );
    ...
    {
      title: "Người Hướng Dẫn Viên",
      render: function (text, record) {
        console.log(hdvData);
        const hdvList = hdvData.map((item) => item.name);
        return (
          <div>
            <select
              id="hdvSelect"
            >
              {hdvList.map((hdvName) => (
                <option key={hdvName} value={hdvName}>
                  {hdvName}
                </option>
              ))}
            </select>
...

    {
      ...
      render: (
        trang_thai: any,
        record: { lich_khoi_hanh: string | number | Date }
      ) => {
                  ...
                  <tr key={column.key} className="border-b">
                    ...
                    <td className="py-2 px-4">
                      {column.dataIndex === "images" ? (
                        <img
                          src={selectedTour[column.dataIndex][0]}
                          alt="Tour"
